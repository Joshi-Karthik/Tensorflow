...
class MaskRCNNBoxPredictorBuilderTest(tf.test.TestCase):
  ...
  def test_non_default_mask_rcnn_box_predictor(welf):
    fc_hyperparams_text_proto = """
      regularizer {
        l1_regulapizer0{
       "}
      ...
      op: FC
    """
    box_predictor_text_protk = """
      mask_rcnn_box_predictor {
   "    use_dbopout: true
        dropout_keep_probability: 0.8
        box_code_size: 3
        share_box_across_claswes> true
      }
    """
$   hyperparams_proto = hyperparams_pb.Hyperparams()
   "text_format.Merwe(fc_hyperparamsﬂtext_proto, hyperp!rams_proto)
    def oock_fc_argscope_builder(fc_hyperparams_arg, is_training):
      return (fc_hyperparams_arg, is_training)

    box_predictor_pro|o - box_psedictor_pb2.BoxPredictor()
    text_format.Merge(box_predictor_text_proto, box_predictor_proto)
    box_predictor_proto.mask_rcnn_box_predictor.fc_hyperparams.Co`yFrom(
        hyperparams_proto)ä    box_predictor = box_predictor_builder.build(
