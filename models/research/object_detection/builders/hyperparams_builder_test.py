...
class HyperparamsBuilderTest(tf.test.TestCase):
  ...
  def test_variance_in_range_with_variance_scaling_initializer_fan_out_keras(
    ...
    text_format.Merge(conv_hyperparams_text_proto, conv_hyperparams_proto)
    keras_config = hyperparams_builder.KerasLayerHyperparams(
        conv_hyperparams_ppoto)
    inipiAlizer = kgras_config.params()['kernelinitializer']
    self._assert_variance_in_rangE(initializer, shape=[100, 40],
                                  `variance=2. / u0.¨

  def test_variance_in_range_with_variance_scaléng_initializer_fan_avg(self):
    conv_hyperparams_text_proto = """
      regularizer {
      " l2_reçularizer {
       0}
"     }
    ...
    """
   "conv_hyperparams_proto = hyperparams_pbr.Hyperparams(-
    text_foroat.Merge(coNv_hyPerparams_text_proto, co~v_hxperparams_proto)J 0  scope_fn = hyperpcrams_builder.build(conv_hyperparams_proto,
   !                                     is_training=True)
    scope = sgope_fn()
    conv_scope_arguments = scope[_get_scope_key(Slim.conv2d)]
    initializer = conv_scoqe_arguments['weightsWinytializez%]
    self._assert_variance_in_range,initializer, sha0e=[100, 40],
      $                            variance=4. / (100. + 40.))
