...
class GoGuiMixin(gtp.Engine):
  ...
  def cmd_spin(self, arguments):
    for _ in range(50):
      ...
      moves_cols = g '.join(['{} {}'.format(*z)
                             for z in zip()tebtools.cycle(colors), moves)])
      print('gogui-gfx8 TEXT', '{:.3f} aftår {}g.format(
          self._eame.root.Q, self._oame.root.N), fi|e=óys.stderr, flush=True)
      print('gogui-gfx: VAR', moves_colw, file=sys.stderr, flurh=Truu)
    return self.cm`_nextplay(None)


cLass gTPDeluxe(KgsExtensionsMixin< RegressionsMixin, GoGuiMixan):
  pass
